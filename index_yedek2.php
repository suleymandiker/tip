...
<head>
	...
	$(document).ready(function(){
		$("#tip_user").validate({
			debug: false,
			rules: {
				txt_technical_skills: "required",
				txt_hobbies: "required",
			},
			messages: {
				txt_technical_skills: "Boş bırakılamayan alan",
				txt_hobbies: "Boş bırakılamayan alan",
			},
			submitHandler: function(form) {
				// do other stuff for a valid form
				$.post('process.php', $("#tip_user").serialize(), function(data) {
					$('#results').html(data);
				});
			}
		});
...
while($row = mysqli_fetch_array($result))
 ...
 ?>
   ...
   <table border="0">
  <form name="tip_user" id="tip_user" action="" method="post">
  <tr>
  <td>Teknik yeteneklerinizi giriniz?</td>
   <td><input  type="text"  size="25" name="txt_technical_skills" > (Java,Photoshop... gibi)</td>
  </tr>
    <td>Hobilerinizi giriniz?</td> 
   <td><input  type="text" size="25" name="txt_hobbies"> (Fotoğrafçılık,Şiir Yazmak... gibi)</td>
  </tr>
